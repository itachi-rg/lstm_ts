...
with tf.Session() as sess:
    ...
    while iter<iterations:
        ...
        if iter %100==0:
            ...
            los=sess.run(loss,feed_dict={x: X_train_samples, y: y_train_samples})
            #print("Accuracy ",acc)
            saver.save(sess, "/home/rgangaraju/lstm/cp1/model.ckpt", global_step = iter)
            print(iter, " : iteration , Loss :", los)

        iter=iter+1
    
    train_preds, train_loss=sess.run([prediction,loss],feed_dict={x: X_train_samples, y: y_train_samples})
    test_preds, test_loss=sess.run([prediction,loss],feed_dict={x: X_test_samples, y: y_test_samples})
    #print train_preds
    #print test_preds

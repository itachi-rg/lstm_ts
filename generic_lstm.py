...
def train(training_file, iterations=5000, time_steps=50, num_lstm_hidden_units=128, num_features=1, num_classes=1, batch_size=1024, learning_rate=0.001, split_percent=0.8, print_iter=100):
    ...
    split_length = file_vector_size+time_steps
    ...
    print("X_samples shape repeated ", X_samples.shape)

    X_samples = np.zeros([num_samples, split_length, file_vector_size+1])
    y_samples = np.zeros([num_samples, 1])

    print("X_samples shape ", X_samples.shape)

    for i in range(num_samples) :
        X_samples[i] = np.append(file_vector_repeat[i],shifted_dataframe_narray[i:i+split_length,0],axis=1)
...
def getbin(value, width):
    return format(value, "{0:0"+str(width)+"b}")
